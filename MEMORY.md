...
## 表单布局规范（2026-05-22 确立）

**铁律：表单字段禁止每个一行死板堆叠。** 必须用 `ProForm.Group` + `colProps` 实现多列紧凑布局：
- 2 字段/行 → `colProps={{ span: 12 }}`
- 3 字段/行 → `colProps={{ span: 8 }}`
- 4 字段/行 → `colProps={{ span: 6 }}`
- 查询行（input + select + button）仍可用 `Space`

适用所有页面：新建/编辑弹窗、录入表单、筛选区。
