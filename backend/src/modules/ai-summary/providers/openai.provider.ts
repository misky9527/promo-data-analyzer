...
export class OpenAIProvider implements IAiProvider {
  ...
  constructor() {
    if (!this.defaultApiKey) {
      this.logger.warn('OPENAI_API_KEY 未设置，OpenAI provider 将不可用');
    }
  }
  ...
  async analyze(
    ...
    const response = await client.chat.completions.create({
      ...
      ...(this.isReasoningOrGpt5(model) ? {} : { temperature: 0.3 }),
      ...(thinkingLevel !== 'off'
    ...
    this.logger.log(`OpenAI 分析完成，返回 ${content.length} 字符`);
    return content;
  }
  ...
  private isReasoningOrGpt5(model: string): boolean {
