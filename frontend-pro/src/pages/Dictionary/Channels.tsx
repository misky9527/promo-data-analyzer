    ...
    { title: '删除时间', dataIndex: 'deletedAt', valueType: 'dateTime', search: false },
    {
      ...
      render: (_, row) => [
        ...
        <Popconfirm
          ...
          onConfirm={async () => {
            await permanentDeleteChannel(row.id);
    ...
    <PageContainer header={{ title: false }}>
      <ProTable<any>
        ...
        toolBarRender={() => [
          ...
          <Button key="add" type="primary" onClick={() => setEditing({})}>新建渠道</Button>,
        ]}
      />
      <ModalForm
      ...
      >
        <ProTable<any>
          ...
          request={async (params) => {
            const result = await getRecycleChannels({ page: params.current, pageSize: params.pageSize });
            return { data: result.list || [], success: true, total: result.total || 0 };
          }}
          search={false}
          options={false}
